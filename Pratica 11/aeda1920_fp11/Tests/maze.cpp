...
void Maze::buildRandomMaze() {
    ...
    while(this->maze.getNumberOfSets() != 1) {
        pos = rand() % (nrows * ncols);//gera um numero qq possivel no labirinto
        neighbours = getNeighbours(pos);//vai buscar os vizinhos desse numero
        new_pos = neighbours[rand() % neighbours.size()];//gera um vizinho qq desse numero
        find1 = maze.find(pos);
