...
bool User::operator<(const User& u1) const {
	//TODO:
    if((getReading().first.empty() && u1.getReading().first.empty()) || (!getReading().first.empty() && !u1.getReading().first.empty())){
        return getReadings().size()<u1.getReadings().size();
    }
    else if(!getReading().first.empty() && u1.getReading().first.empty()){
        return getReadings().size()+1<u1.getReadings().size();
    }
    else if(getReading().first.empty() && !u1.getReading().first.empty()){
        return getReadings().size()<u1.getReadings().size()+1;
    }
}
