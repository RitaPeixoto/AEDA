...
TEST(test_1, test_c) {
	...
	EXPECT_EQ("Joao", bPtr3->getReader()->getName());
	...
	EXPECT_EQ("Maria", bPtr1->getReader()->getName());
...
TEST(test_1, test_d) {
	...
	EXPECT_EQ(1, ur1.size());
	EXPECT_EQ("t.silva@gmail.com", ur1[0].getEMail());
	...
	ur1 = rc1.getUserRecords();
	EXPECT_EQ(4, ur1.size());

	rc1.addUserRecord(&u6);
	ur1 = rc1.getUserRecords();
	EXPECT_EQ(5, ur1.size());
...
TEST(test_1, test_e) {
	...
	EXPECT_EQ("tiago.sousa@gmail.com", urs1[0].getEMail());
	...
	for(int i = 0; i < urs2.size(); i++) {
		if(urs2[i].getName() == "Ana") EXPECT_EQ("a.gomes@gmail.com", urs2[i].getEMail());
	}
...
TEST(test_1, test_f) {
	...
	EXPECT_EQ(1, rc1.getBestReaderCandidates().size());

	ReadingClub rc2;
	rc2.addBestReaderCandidates(candidates_0, 2);
	EXPECT_EQ(0, rc2.getBestReaderCandidates().size());

	ReadingClub rc3;
	rc3.addBestReaderCandidates(candidates_5, 3);
	EXPECT_EQ(2, rc3.getBestReaderCandidates().size());
	...
	EXPECT_EQ(1, rc4.getBestReaderCandidates().size());

	ReadingClub rc5;
	rc5.addBestReaderCandidates(candidates_b, 5);
	EXPECT_EQ(1, rc5.getBestReaderCandidates().size());
...
TEST(test_1, test_g) {
	...
	EXPECT_EQ(0, rc1.awardReaderChampion(champion));
	...
	EXPECT_EQ(0, rc2.awardReaderChampion(champion));
	...
	EXPECT_EQ(3, rc3.awardReaderChampion(champion));
	EXPECT_EQ("Pedro", champion.getName());
	...
	EXPECT_EQ(2, rc4.awardReaderChampion(champion));
