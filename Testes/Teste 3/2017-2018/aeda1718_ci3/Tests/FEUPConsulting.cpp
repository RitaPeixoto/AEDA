...
void FEUPConsulting::addAvailability(Student* student, string expertizeAvailable, unsigned cost) {
	...
	Expertize exp= expertizes.find((Expertize(expertizeAvailable,cost)));
	if (exp== Expertize("",0)){
	    Expertize new_e (expertizeAvailable,cost);
	    new_e.addConsultant(student);
	    expertizes.insert(new_e);
	}
	else{
...
vector<Student*> FEUPConsulting::getCandidateStudents(Project* project) const {
	...
	while(!(it.isAtEnd())) {
        if (it.retrieve().getName() == (*project).getExpertize()) {
            if (it.retrieve().getCost() == (*project).getCost()) {
...
bool FEUPConsulting::assignProjectToStudent(Project* project, Student* student) {
	...
	vector<Student*> aux=getCandidateStudents(project);
    vector<Student*>::iterator it = aux.begin();
...
int FEUPConsulting::mostActiveStudent(Student& studentMaximus) {
	//TODO:
    priority_queue<Student> st = getActiveStudents();
    if(st.size() <= 1){
        return 0;
    }
    Student aux = st.top();
    st.pop();
    Student aux1= st.top();
    if(aux.getPastProjects().size() == aux1.getPastProjects().size()){
        return 0;
    }
